            ...
            {MOCK_HOLDERS.map((holder) => (
                <div key={holder.rank} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-3">
                        ...
                        {holder.tag === 'DEV' && <span className="text-[10px] bg-blue-500/20 text-blue-400 px-1.5 rounded">DEV</span>}
                        {holder.tag === 'BONDING CURVE' && <span className="text-[10px] bg-orange-500/20 text-orange-400 px-1.5 rounded">BONDING CURVE</span>}
                    </div>
